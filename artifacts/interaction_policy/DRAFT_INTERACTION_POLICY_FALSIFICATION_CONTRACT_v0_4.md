# Interaction Policy Experiment — Falsification Contract v0.4 (DRAFT)

**Status:** DRAFT — revised after Contract Attack Round 3; not frozen, implemented against the sealed set, or executed.

This version closes the main remaining mechanism-confound identified in v0.3: a `STATE-ON` artifact can change behavior merely because an extra prompt artifact is present. v0.4 therefore separates **learned content**, **artifact presence**, and **no-state** effects.

A branch-level positive result requires both:

1. **mechanism evidence** that feedback-derived policy content changes later decisions in the predicted, scope-sensitive direction; and
...
- reduces human AI-operation burden by a practically meaningful amount;
...
Same as A, plus persistent feedback-derived interaction-policy state rendered under the frozen schema below.

### C — Simple static heuristic baseline
Same as A, plus a **fixed, non-learning interaction policy** frozen before sealed evaluation. It may encode generic interaction heuristics and safety rules but may not update from participant feedback or contain user-specific learned conventions.

Condition C exists to test whether any apparent B advantage actually requires online adaptation rather than a simpler static rule.
...
The persisted adaptive state may contain only generic interaction-policy abstractions such as:
...
This replication is mechanism-only. Human-facing tasks remain one-shot.
...
For B to show a practically meaningful H1 advantage, it must satisfy both:

1. `H1_turns_B <= 0.80 * min(H1_turns_A, H1_turns_C)`; and
2. `H1_chars_B <= 1.10 * min(H1_chars_A, H1_chars_C)`.

Thus a trivial one-turn difference is not sufficient, and burden cannot simply be compressed into much longer user messages.
...
2. B satisfies the H1 practical-effect threshold against **both A and C**;
...
- H1 fails the pre-frozen practical-effect threshold;
...
- the adaptive condition reduced operating burden by the frozen threshold without worsening measured trajectory quality;
...
Before the sealed set may be targeted by implementation or executed, freeze:

- policy-state schema and complexity bound;
- canonical renderer;
- development-only context taxonomy;
- semantic-leakage review;
- updater input contract;
- A/B/C condition definitions;
- fixed heuristic C;
- four learning triads;
...
- blind coding procedure;
- participant response policy;
- H1 20% practical-effect threshold and character/component guards;
- H2 coding rules;
